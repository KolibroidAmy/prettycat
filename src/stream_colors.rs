...
use crate::console::{for_each_console_element, parse_ansi_type, AnsiCodeType, Color, ConsoleElem};
...
impl<T> StreamColorizer for T
where
    T: PositionalRecolorizer,
{
    fn copy_colorized<I, O>(
        &mut self,
        input: I,
    ...
    <<T as GenericImageView>::Pixel as Pixel>::Subpixel: Into<u8>,
{
    fn get_color(&mut self, (x, y): (usize, usize)) -> Color {
        let pixel = self
            .img
            .get_pixel(x as u32 % self.img.width(), y as u32 % self.img.height());
